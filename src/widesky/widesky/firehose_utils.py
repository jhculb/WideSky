...
def read_dag_cbor(stream: t.IO[bytes]) -> t.Any:
    ...
    if major_type == 0:  # Unsigned integer
        ...
        if additional_info == 20:  # False
            ...
            raise ValueError(f"Unsupported simple value with additional info {additional_info}.")
    else:
...
def read_cbor_uint(stream: t.IO[bytes], additional_info: int) -> int:
    ...
    if additional_info < 24:
        ...
        raise ValueError(f"Unsupported additional information for integer parsing: {additional_info}.")
